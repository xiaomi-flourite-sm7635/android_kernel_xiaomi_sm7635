load(":gen3auto.bzl", "define_gen3auto")
load(":autogvm.bzl", "define_autogvm")
load(":autoghgvm.bzl", "define_autoghgvm")
load(":autogvm_lxc.bzl", "define_autogvm_lxc")
load(":gen4auto_lxc.bzl", "define_gen4auto_lxc")
load(":sdmsteppeauto.bzl", "define_sdmsteppeauto")
load(":pineapple.bzl", "define_pineapple")
load(":niobe.bzl", "define_niobe")
load(":pineapple_tuivm.bzl", "define_pineapple_tuivm")
load(":pineapple_oemvm.bzl", "define_pineapple_oemvm")
load(":pineapple_vms.bzl", "define_pineapple_vms")
load(":pineapple_allyes.bzl", "define_pineapple_allyes")
load(":niobe_tuivm.bzl", "define_niobe_tuivm")
load(":msm_common.bzl", "define_signing_keys")
load("//build:msm_kernel_extensions.bzl", "define_top_level_rules")
load(":shennong.bzl", "define_shennong")
load(":manet.bzl", "define_manet")
load(":zorn.bzl", "define_zorn")
load(":aurora.bzl", "define_aurora")
load(":houji.bzl", "define_houji")
load(":ruyi.bzl", "define_ruyi")
load(":suiren.bzl", "define_suiren")
load(":goku.bzl", "define_goku")
load(":peridot.bzl", "define_peridot")
load(":chenfeng.bzl", "define_chenfeng")
load(":uke.bzl", "define_uke")
load(":muyu.bzl", "define_muyu")
load(":amethyst.bzl", "define_amethyst")
load(":flourite.bzl", "define_flourite")
load(":blair.bzl", "define_blair")
load(":xiaomi_sm8650_common.bzl", "export_xiaomi_headers")
load(":pitti.bzl", "define_pitti")
load(":anorak.bzl", "define_anorak")

def define_msm_platforms():
    define_top_level_rules()
    define_signing_keys()
    define_gen3auto()
    define_autogvm()
    define_autoghgvm()
    define_autogvm_lxc()
    define_gen4auto_lxc()
    define_sdmsteppeauto()
    define_pineapple()
    define_niobe()
    define_pineapple_tuivm()
    define_pineapple_oemvm()
    define_pineapple_allyes()
    define_pineapple_vms()
    define_shennong()
    define_manet()
    define_zorn()
    define_aurora()
    define_houji()
    define_ruyi()
    define_suiren()
    define_goku()
    define_peridot()
    define_chenfeng()
    define_uke()
    define_muyu()
    define_amethyst()
    define_flourite()
    define_blair()
    define_pitti()
    export_xiaomi_headers()
    define_niobe_tuivm()
    define_anorak()
