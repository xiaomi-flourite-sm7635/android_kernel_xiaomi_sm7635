...
static inline bool is_app_collec_pids(int pid)
{
	...
	for (i = 0; i < g_active_package.pids_num; i++) {
		if (g_active_package.pids[i] == pid) {
			//MI_LB_DBG("pid %d is in pids list\n", pid);
			return true;
		}
	}
				    ...
				    pgoff_t start, unsigned count, bool is_fault)
{
	...
	if (g_active_package.is_tgid) {
		if (g_active_package.current_package == NULL || !is_app_collec_pids(current->tgid))
			return;
	} else if (g_active_package.current_package == NULL || !is_app_collec_pids(current->pid))
		return;
...
static void mi_lb_del_lists_file_info(struct package_info *package)
{
	...
	list_for_each_entry_safe(file_entry, tmp, &package->file_infos, list) {
		...
		if(!is_file_info_collected(file_entry))	{
			file_entry->i_count--;
			//MI_LB_DBG("file %s not collected, --i_count %d ", file_entry->path, file_entry->i_count);
			if(file_entry->i_count == 0) {
				free_one_fileinfo(file_entry);
			}
		}else {
...
static bool mb_lb_dealwith_collect_finish(struct lb_owner *owner)
{
	...
	if ((package->owner.uid == owner->uid) &&
		...
		cancel_delayed_work_sync(&g_manager->record_work);
		wake_up_all(&g_manager->wait);
	} else {
...
static void mi_lb_do_record(struct work_struct *work)
{
	wake_up_all(&g_manager->wait);
}
...
static bool mb_lb_dealwith_collect_start(struct lb_owner *owner)
{
	...
	if (!package) {
		if (g_manager->app_num >= MAX_APPS_LIST) {
			...
			MI_LB_ERR("app list num is too much delete oldest app: %s ", oldest_package->owner.package_name);
	...
	if (!strncmp("com.alibaba.android.rimet",
                     owner->package_name,strlen(owner->package_name))) {
		g_active_package.is_tgid = true;
	} else {
		g_active_package.is_tgid = false;
	}
	...
	schedule_delayed_work(&g_manager->record_work, msecs_to_jiffies(2000));
	return 0;
}
...
{
	int ret = 0;
	MI_LB_DBG("cmd: %d, uid: %d, pid: %d, packagename: %s",
	   		cmd, owner->uid, owner->pid, owner->package_name);
	switch (cmd) {
	case APP_COLLECT_START:
		if (atomic_add_unless(&collecting_, 1, 1)) {
			...
			struct package_info *package = g_active_package.current_package;
			if(!strcmp(package->owner.package_name, owner->package_name)) {
				MI_LB_DBG("app:%s add pid[%d] to pids", owner->package_name, owner->pid);
				add_pid_to_pids(owner->pid);
			}
			else {
				MI_LB_DBG("pre app[%s] is collecting next app[%s]", package->owner.package_name,
							owner->package_name);
			}
		}
		break;
					...
					struct file *file, struct file_info **file_info, char *f_path)
{
	...
	list_for_each(p, &active_package->file_infos) {
		...
		//file_path(file, g_manager->file_path, PATH_MAX);
		if (file_list->s_dev ==  file_inode(file)->i_sb->s_dev
			 //&& file_list->i_ino == file_inode(file)->i_ino) {
					...
					struct file *file, struct file_info **file_info)
{
	...
	if (find) {
		...
		if(info->i_ino != file_inode(file)->i_ino
			...
			MI_LB_DBG("file %s has been modified, delete this file info", f_path);
	...
	for (i = 0; i < g_manager->cur; i++) {
		...
		if (mi_lb_get_fileinfo(package, collect_info->file, &file_info) ==0) {
			if ((collect_info->start + collect_info->len) <= file_info->i_size) {
				...
				package->file_size += blocks << PAGE_SHIFT;
				package->last_size += blocks << PAGE_SHIFT;
			}
		}
	}
}
...
static void mi_lb_compiler_do_work(struct lb_manager *manager)
{
	...
	last_size = package->last_size;
	package_info_put(package);
	atomic_set(&collecting_, 0);
	mutex_unlock(&g_manager->compiler_lock);
	MI_LB_DBG("exit this collect size %u total_memory %lu apps %u",
			last_size, g_manager->total_memory , g_manager->app_num);
}
...
static int mi_lb_compiler_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		DEFINE_WAIT(entry);
		prepare_to_wait(&manager->wait, &entry, TASK_UNINTERRUPTIBLE);
		schedule();
		finish_wait(&manager->wait, &entry);
...
int mi_lb_init_manager(void)
{
	...
	rwlock_init(&g_manager->package_list_rw_lock);
	INIT_LIST_HEAD(&g_manager->package_infos);
	INIT_DELAYED_WORK(&g_manager->record_work, mi_lb_do_record);
	g_manager->cur = 0;
	g_manager->collected_num = 0;
	init_waitqueue_head(&g_manager->wait);
