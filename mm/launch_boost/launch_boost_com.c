...
int mi_lb_add_package_info(struct package_info *package)
{
	...
	if ((package_list->owner.uid == package->owner.uid) 
		&& !strncmp(package_list->owner.package_name, 
			package->owner.package_name, strlen(package->owner.package_name))) {
#ifdef CONFIG_PID_FILTER 
		package_list->owner.pid =  package->owner.pid;
#endif
		goto unlock;
	}
	read_unlock(&g_manager->package_list_rw_lock);
	write_lock(&g_manager->package_list_rw_lock);
	list_add(&package->app_list, &g_manager->package_infos);
	package_info_get(package);
	g_manager->app_num++;
	write_unlock(&g_manager->package_list_rw_lock);
	return 0;
unlock:
	read_unlock(&g_manager->package_list_rw_lock);
	return 0;		
...
struct package_info *mi_lb_get_package_info(struct lb_owner *owner, bool move_to_head)
{
	...
	list_for_each(pos, head) {
		...
		if ((package_list->owner.uid == owner->uid) && 
			!strncmp(package_list->owner.package_name, owner->package_name,
