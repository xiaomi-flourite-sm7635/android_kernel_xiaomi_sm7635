...
{
	struct lb_owner *owner = &app_preread_info->owner;
	app_preread_file_info *preread_file_info;
	int i = 0;
	unsigned long start = 0, end = 0, last = 0;

	MI_LB_DBG("uid: %d, pid: %d, packagename: %s", owner->uid, owner->pid, owner->package_name);
	MI_LB_DBG("preread files num: %d buffer size %ld", app_preread_info->files,
							app_preread_info->file_info_buf_size);

	preread_file_info = &app_preread_info->preread_file_info[0];
	for (i = 0; i < app_preread_info->files; i++) {
			last = preread_file_info->bitmap_longs * sizeof(unsigned long) * 8;
			start = 0;
			end = 0;
			for (;;) {
				start = find_next_bit(preread_file_info->bitmaps, last, end);
				if (start >= last)
					break;
				end = find_next_zero_bit(preread_file_info->bitmaps, last, start);
				MI_LB_DBG("bitmapfind start %ld  end %ld", start, end);
			}
		preread_file_info = (app_preread_file_info *)((char*)preread_file_info
					+ sizeof(app_preread_file_info)
					+ preread_file_info->bitmap_longs * sizeof(unsigned long));
	}
}
#endif
...
int mi_lb_dealwith_preread_size_cmd(struct lb_app_preread_info_size *app_preread_info_size)
{
	...
	MI_LB_DBG("preread size: package uid: %d, pid: %d, packagename: %s",
				app_preread_info_size->owner.uid,
				app_preread_info_size->owner.pid,
				app_preread_info_size->owner.package_name);
...
int mi_lb_dealwith_preread_info_cmd(struct lb_app_preread_info *app_preread_info)
{
	...
	MI_LB_DBG("preread info uid: %d, packagename: %s",
				app_preread_info->owner.uid,
				app_preread_info->owner.package_name);
