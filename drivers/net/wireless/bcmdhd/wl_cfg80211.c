	...
	struct wireless_dev *wdev, wl_iftype_t iftype, bool pm_block, u8 bssidx, u8 ifidx)
{
	...
	if (ndev) {
		pub = (dhd_pub_t *) dhd_get_pub(ndev);
		xr_role = DHD_GET_XR_ROLE(pub);
	...
	enum wl_pm_workq_act_type type)
{
	...
	if (wq_duration) {
#endif /* BCMDONGLEHOST && OEM_ANDROID */
		if (dhd_sched_delayed_work_on(DHD_KWORKER_CPUBITMAP, &cfg->pm_enable_work,
	...
	struct wireless_dev *wdev, char *ifname)
{
	...
	if (if_wdev->iftype == NL80211_IFTYPE_P2P_DEVICE) {
		mutex_lock(&cfg->if_sync);
	...
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data)
{
	...
	WL_ERR(("EVENT: EXT_AUTH_REQ received\n"));
	...
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data)
{
	...
	WL_ERR(("EVENT: mgmt tx status: event %s, status=%d _net_info->mgmt_txstatus=%lu\n",
		event_type == WLC_E_MGMT_FRAME_TXSTATUS ?
		"MGMT_FRAME_TXSTATUS" : "MGMT_FRAME_OFF_CHAN_COMPLETE", e->status, _net_info->mgmt_txstatus));

	if (!test_bit(MGMT_TX_SEND_FRAME, &_net_info->mgmt_txstatus) ||
		(*packetid != _net_info->mgmt_txid)) {
		WL_ERR(("id not match: packetid=%d, mgmt_txid=%lu\n", *packetid, _net_info->mgmt_txid));
		return 0;
	}
	...
	if (event_type == WLC_E_MGMT_FRAME_TXSTATUS) {
		if (status == WLC_E_STATUS_SUCCESS) {
			...
			set_bit(MGMT_TX_NOACK,
				&_net_info->mgmt_txstatus);
			WL_ERR(("set MGMT_TX_NOACK : _net_info->mgmt_txstatus=%lu\n", _net_info->mgmt_txstatus));
		}
	} else {
...
static void wl_cfg80211_wait_for_disconnection(struct bcm_cfg80211 *cfg, struct net_device *dev)
{
	...
	if (!wait_cnt && wl_get_drv_status(cfg, DISCONNECTING, dev)) {
		...
		if (wl_get_drv_status(cfg, CONNECTED, dev)) {
			WL_ERR(("no response from firmware, force cfg80211_disconnected\n"));
...
wl_cfg80211_afx_handler(struct work_struct *work)
{
	...
	struct bcm_cfg80211 *cfg;
	s32 ret = BCME_OK;
	WL_ERR(("wl_cfg80211_afx_handler in\n"));
...
wl_cfg80211_af_searching_channel(struct bcm_cfg80211 *cfg, struct net_device *dev)
{
	...
	while ((cfg->afx_hdl->retry < max_retry) &&
		...
		WL_ERR(("Scheduling the action frame for sending.. retry %d\n",
			cfg->afx_hdl->retry));
		/* search peer on peer's listen channel */
		dhd_sched_kworker_on(DHD_KWORKER_CPUBITMAP, &cfg->afx_hdl->work);
		...
		if (cfg->afx_hdl->my_listen_chan) {
			WL_ERR(("Scheduling Listen peer in my listen channel = %d\n",
 ...
 */
			...
			*cookie = (u64)mf_params->data;
			...
			WL_ERR(("Auth frame, cookie=%lld, fc=%x, len=%d, channel=%d\n",
				*cookie, mf_params->fc,
				le16_to_cpu(mf_params->len),
				mf_params->channel));
			...
			if (!test_bit(MGMT_TX_NOACK, &netinfo->mgmt_txstatus) &&
				...
				WL_ERR(("TX auth frame operation is success\n"));
				ack = true;
			} else {
				ack = false;
				WL_ERR(("TX auth frame operation has failed, txstatus %ld , timeout %d\n",
					netinfo->mgmt_txstatus, timeout));
				if (test_bit(MGMT_TX_NOACK, &netinfo->mgmt_txstatus)) {
					s32 ret = 0;
	...
	struct cfg80211_ap_settings *info)
{
	...
	dhd_pub_t *dhd = (dhd_pub_t *)(cfg->pub);
#endif /* WL_DHD_XR */
#ifdef ABORT_ONGOING_ASSOCIATION_AT_START_AP
	struct net_device *ndev = bcmcfg_to_prmry_ndev(cfg);
#endif /* ABORT_ONGOING_ASSOCIATION_AT_START_AP */
	...
	if (IS_STA_IFACE(ndev_to_wdev(ndev)) &&
		...
		/* Driver waits a link up event for 10 secs (MAX_AP_LINK_WAIT_TIME).
		 * Makes a bus error when the event isn't posted during the time.
		 * The link up event can be delayed more than 10 secs when
		 * association procedure takes long time.
		 * To prevent this issue, abort onoing connection at starting soft ap.
		 */
		WL_ERR(("force to abort ongoing connection\n"));
	...
	const wl_event_msg_t *e, void *data)
{
	 ...
	 * event
	 */
	dhd_sched_delayed_work_on(DHD_KWORKER_CPUBITMAP, &cfg->hogsqs_eventwork,
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
		...
		WL_ERR(("EVENT: auth frame rx received\n"));
	...
	struct cfg80211_chan_def *chandef)
{
	...
	for (i = 0; i < DHD_NUM_STATIC_IFACES; i++) {
		if (cfg && (cfg->static_ndev[i] == ndev)) {
			if (cfg->static_ndev_state[i] != NDEV_STATE_FW_IF_CREATED) {
				WL_ERR(("FW interface %s is not created in ndev.\n", cfg->static_ndev[i]->name));
				err = -EIO;
				return err;
			}
		}
...
wl_cfg80211_set_sae_pwe(struct net_device *ndev, u8 sae_pwe)
{
	...
	if (ret < 0)
		WL_ERR(("Failed set SAE PWE(%d), ret=%d\n",sae_pwe, ret));
	else
		WL_DBG(("Successed set SAE PWE=%d\n", sae_pwe));
