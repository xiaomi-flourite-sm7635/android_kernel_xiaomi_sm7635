...
static void wl_cfg80211_bt_timerfunc(ulong data)
{
	...
	WL_ERR(("Enter\n"));
	bt_local->timer_on = 0;
	dhd_sched_kworker_on(DHD_KWORKER_CPUBITMAP, &bt_local->work);
}
...
static void wl_cfg80211_bt_handler(struct work_struct *work)
{
	...
	WL_ERR(("wl_cfg80211_bt_handler in\n"));
	...
	if (powermode_val == '1') {
		...
		if (btco_inf->timer_on) {
			...
			/* need to restore original btc flags & extra btc params */
				WL_ERR(("bt->bt_state:%d %d\n", btco_inf->bt_state, __LINE__));
				/* wake up btcoex thread to restore btlags+params  */
				dhd_sched_kworker_on(DHD_KWORKER_CPUBITMAP, &btco_inf->work);
			}
		}
