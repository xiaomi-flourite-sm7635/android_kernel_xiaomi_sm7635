...
uint dhd_ssr_or_suspend_state = FALSE;
module_param(dhd_ssr_or_suspend_state, uint, 0644);
...
dhd_set_mac_address(struct net_device *dev, void *addr)
{
	...
	if (dhd_is_static_ndev(&dhd->pub, dev) &&
		static_if_ndev_get_state(cfg, dev) != NDEV_STATE_FW_IF_CREATED) {
...
#pragma GCC diagnostic pop
	...
	if (ret > 0) {
		dhd_sched_delayed_work_on(DHD_KWORKER_CPUBITMAP, &(dhd)->event_log_dispatcher_work,
...
#else
	dhd_sched_delayed_work_on(DHD_KWORKER_CPUBITMAP, &dhd->event_log_dispatcher_work, 0);
#endif /* DHD_USE_KTHREAD_FOR_LOGTRACE */
	return;
}
...
#ifdef SSR_TEST_CMD
void dhd_hang_trigger_test(struct net_device *net)
{
	dhd_info_t *dhd = DHD_DEV_INFO(net);

	if(disable_bug_on == FALSE)
	{
		DHD_ERROR(("set disable_bug_on TRUE "));
		disable_bug_on = TRUE;

		DHD_ERROR(("trigger hang test, error ETIMEDOUT"));
		dhd_check_hang(net, &dhd->pub, -ETIMEDOUT);

		DHD_ERROR(("set disable_bug_on FASLE"));
		disable_bug_on = FALSE;
	}
	else
	{
		DHD_ERROR(("trigger hang test, error ETIMEDOUT"));
...
struct net_info *iter, *next;
	...
	if (dhd_ssr_or_suspend_state == FALSE) {
		DHD_ERROR(("skip chip reset.\n"));
		skip_reset = true;
		wl_cfg80211_sta_ifdown(net);
		goto exit;
	} else {
		DHD_ERROR(("ssr or suspend coming in\n"));
	}
...
#if defined(WL_DHD_XR_PCISDIO)
	if (DHD_GET_XR_ROLE((&dhd->pub)) != XR_ROLE) {
		// Make sure if pub is right for interface.
		DHD_ERROR(("%s invalid handle! %s\n", __FUNCTION__,  net->name));
		dump_stack();
		return -EINVAL;
	}
#endif // endif
	...
	if (!success) {
		...
		if (type == ETHER_TYPE_802_1X) {
			if (dhd_is_4way_msg((uint8 *)eh) == EAPOL_4WAY_M4) {
				...
				dhd_sched_delayed_work_on(DHD_KWORKER_CPUBITMAP, &ifp->m4state_work,
