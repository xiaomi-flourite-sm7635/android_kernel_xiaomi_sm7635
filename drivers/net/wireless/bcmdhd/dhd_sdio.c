...
dhd_bus_rxctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	bus->rxlen = 0;
	dhd_os_sdunlock(bus->dhd);
	if (CTRL_RESCHED > 0 && !rxlen && timeleft == 0) {
		resched_cnt++;
		if (resched_cnt <= CTRL_RESCHED) {
			uint32 status, retry = 0;
			R_SDREG(status, &bus->regs->intstatus, retry);
			if ((status & I_HMB_HOST_INT) || PKT_AVAILABLE(bus, status)) {
				DHD_ERROR(("%s: reschedule dhd_dpc, resched_cnt=%d, status=0x%x\n",
						__FUNCTION__, resched_cnt, status));
				bus->ipend = TRUE;
				bus->dpc_sched = TRUE;
				dhd_sched_dpc(bus->dhd);
...
dhdsdio_readconsole(dhd_bus_t *bus)
{
	...
	DHD_TRACE(("conlog: addr=0x%x, idx=0x%x, last=0x%x \n", c->log.buf,
	   idx, c->last));
...
dhdsdio_advertise_bus_cleanup(dhd_pub_t	 *dhdp)
{
	...
	DHD_ERROR(("%s: bus state:%d \n", __FUNCTION__, dhdp->busstate));
	if (dhdp->busstate != DHD_BUS_DOWN) {
		DHD_LINUX_GENERAL_LOCK(dhdp, flags);
		DHD_ERROR(("%s: making DHD_BUS_DOWN_IN_PROGRESS \n", __FUNCTION__));
