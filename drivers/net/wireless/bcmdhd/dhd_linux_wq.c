...
static void dhd_deferred_sched_work_on(int cpu_integer, struct work_struct *work) {
    int cpu;
    cpumask_t cpu_mask;
	...
	event_handler_t event_handler, u8 priority)
{
	...
	dhd_deferred_sched_work_on(0x6E, (struct work_struct *)deferred_wq);
...
dhd_deferred_work_handler(struct work_struct *work)
{
	...
	DHD_ERROR(("dhd_deferred_work_handler in\n"));
	if (!deferred_work) {
		DHD_ERROR(("%s: work queue not initialized\n", __FUNCTION__));
		return;
	}
