...
exit:
	if (sched) {
		...
		if ((ring_status.written_bytes == ring_status.read_bytes) &&
				(ring_info->interval)) {
			dhd_sched_delayed_work_on(DHD_KWORKER_CPUBITMAP, d_work, ring_info->interval);
		}
	}
	...
	int flags, int time_intval, int threshold)
{
	...
	if (time_intval == 0 || log_level == 0) {
		...
		cancel_delayed_work_sync(&ring_info->work);
		dhd_sched_delayed_work_on(DHD_KWORKER_CPUBITMAP, &ring_info->work, ring_info->interval);
	}
...
dhd_os_trigger_get_ring_data(dhd_pub_t *dhdp, char *ring_name)
{
	...
	if (os_priv) {
		...
		dhd_sched_delayed_work_on(DHD_KWORKER_CPUBITMAP, &ring_info->work, 0);
	} else {
