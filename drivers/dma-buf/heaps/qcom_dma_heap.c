...
unsigned long *dmabuf_hugetlb_symbol_contpte_map;
...
{
	if (!dmabuf_hugetlb_symbol_pmd_map || !dmabuf_hugetlb_symbol_pmd_zap || !dmabuf_hugetlb_symbol_pmd_split || !dmabuf_hugetlb_symbol_contpte_map)
		return -1;

	return sysfs_emit(buf, "pmd map: %lu	contpte map: %lu	zap: %lu	split: %lu\n", *dmabuf_hugetlb_symbol_pmd_map, *dmabuf_hugetlb_symbol_contpte_map, *dmabuf_hugetlb_symbol_pmd_zap, *dmabuf_hugetlb_symbol_pmd_split);
}
...
static void search_dmabuf_huge_remap_pfn_range(void)
{
	...
	ret = debug_symbol_lookup_name("dmabuf_hugetlb_contpte_map");
	if (ret == 0) {
		dmabuf_hugetlb_symbol_contpte_map = NULL;
		pr_err("dmabuf_hugetlb_contpte_map can't find.\n");
	} else
		dmabuf_hugetlb_symbol_contpte_map = (void*)ret;
