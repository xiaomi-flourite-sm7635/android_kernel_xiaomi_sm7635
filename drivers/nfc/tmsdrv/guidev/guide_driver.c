...
#endif
{
    ...
    case NXP_PN560:
        ...
        TMS_INFO("NXP pn560 driver start probe\n");
...
static int guidev_parse_dts(struct guide_dev *gdata)
{
    ...
    gdata->hw_res.clkreq_gpio = of_get_named_gpio(np, "tms,clkreq-gpio", 0);
    TMS_INFO("irq_gpio = %d, ven_gpio = %d, dwnld_gpio = %d, clkreq_gpio = %d, error:%d\n",
              gdata->hw_res.irq_gpio, gdata->hw_res.ven_gpio, gdata->hw_res.download_gpio, gdata->hw_res.clkreq_gpio,
              ret);
    return SUCCESS;
err_ven:
    gpio_free(gdata->hw_res.irq_gpio);
...
static int guidev_gpio_configure(struct hw_resource hw_res)
{
    ...
    if (gpio_is_valid(hw_res.clkreq_gpio)) {
        ret = gpio_direction_input(hw_res.clkreq_gpio);

        if (ret < 0) {
            TMS_ERR("not able to set clkreq gpio as input\n");
            return -ERROR;
        }
    }
...
static int guidev_chip_match(struct guide_dev *gdata)
{
    ...
    for (count = 0; count < ARRAY_SIZE(info_list); count++) {
        ...
        } else if (info_list[count].type == NXP_PN560) {
...
#endif
{
    ...
    case NXP_PN560:
        TMS_INFO("NXP PN560 nfc start removed\n");
...
static int guide_suspend(struct device *device)
{
    ...
    case NXP_PN560:
        TMS_INFO("NXP PN560 nfc start suspend\n");
...
static int guide_resume(struct device *device)
{
    ...
    case NXP_PN560:
        TMS_INFO("NXP PN560 nfc start resume\n");
