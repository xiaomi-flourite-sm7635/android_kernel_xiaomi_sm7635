...
static int nfc_gpio_configure_init(struct nfc_info *nfc)
{
    ...
    if (gpio_is_valid(nfc->hw_res.clkreq_gpio)) {
        ret = gpio_direction_input(nfc->hw_res.clkreq_gpio);

        if (ret < 0) {
            TMS_ERR("Unable to set clkreq gpio as input\n");
            return ret;
        }
    }

    /* Read clkreq GPIO number from device tree*/
    ret = of_property_read_u32_index(nfc->i2c_dev->of_node,
                        "tms,clkreq-gpio", 1, &clkreq_gpio);
    if (ret < 0) {
    ...
    if (ret < 0) {
        TMS_ERR("clkreq gpio %d as wakeup capable failed, ret: %d\n",
                clkreq_gpio, ret);
        return ret;
    } else {
        TMS_INFO("clkreq gpio %d as wakeup capable successfully\n", clkreq_gpio);
    }
...
static int nfc_parse_dts_init(struct nfc_info *nfc)
{
    ...
    nfc->hw_res.clkreq_gpio = of_get_named_gpio(np, "tms,clkreq-gpio", 0);
    if (gpio_is_valid(nfc->hw_res.clkreq_gpio)) {
        rcv = gpio_request(nfc->hw_res.clkreq_gpio, "nfc_clkreq");

        if (rcv) {
            TMS_WARN("Unable to request gpio[%d] as clkreq\n",
                     nfc->hw_res.clkreq_gpio);
        }
    } else {
        TMS_ERR("clkreq gpio not specified\n");
    }

    TMS_DEBUG("NFC device name is %s, count = %d\n", nfc->dev.name,
              nfc->dev.count);
    TMS_INFO("irq_gpio = %d, ven_gpio = %d, download_gpio = %d, clkreq_gpio = %d\n",
             nfc->hw_res.irq_gpio, nfc->hw_res.ven_gpio, nfc->hw_res.download_gpio, nfc->hw_res.clkreq_gpio);
    return SUCCESS;
err_free_irq:
    gpio_free(nfc->hw_res.irq_gpio);
