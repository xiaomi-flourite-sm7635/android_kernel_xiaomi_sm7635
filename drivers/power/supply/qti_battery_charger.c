...
#define XM_QUICK_REV_CHG_READY_NOTIFY	0x61
#define XM_QUICK_REV_CHG_OVER_NOTIFY	0x62
...
#define DFX_ID_XM_PPS_CHARGE_SLOWLY	909001006
#define DFX_ID_NON_XM_STANDARD_CHARGER	909001007
...
#define DFX_ID_CHG_SMART_NAVIGATION	909003006
...
#define DFX_ID_CHG_SMART_NAVIGATION_SOC_ERR 909006011
...
enum xm_chg_debug_type {
	CHG_WLS_DEBUG,
	CHG_ADSP_LOG,
	CHG_CLOUD_DYNAMIC_SHUTDOWN_DATA,
	CHG_UI_SOH_DATA,
	CHG_SLAVE_UI_SOH_DATA,
	CHG_UI_SOH_SN_CODE,
	CHG_UI_SLAVE_SOH_SN_CODE,
	CHG_CLOUD_FOD_DATA,
	CHG_MANUFACTURING_DATE_DATA,
	CHG_FIRST_USAGE_DATE_DATA,
	CHG_DEBUG_TYPE_MAX,
};
...
enum xm_chg_dfx_type {
	...
	CHG_DFX_SMART_ENDU_SOC_ERR,
	CHG_DFX_SMART_NAVI_SOC_ERR,
	CHG_DFX_BATT_AUTH_FAIL,
	CHG_DFX_SOC_NOT_FULL,
	CHG_DFX_XM_PPS_CHARGE_SLOWLY,
	CHG_DFX_NON_XM_STANDARD_CHARGER,
	CHG_DFX_MAX_INDEX,
};
...
static const char *const xm_dfx_chg_report_text[] = {
	...
	"PdAuthFail", "CpEnFail", "chgBattCycle", "SmartEndurance",
	"NotChgInLowTemp","NotChgInHighTemp", "DualBattLinkerAbsent",
	"chgBattAuthFail", "TbatHot", "TbatCold", "AntiFail", "Antiburn",
	"dualVbatDiff", "CpTdieHot","WlsFastChgFail", "WlsQLow", "WlsRxOTP",
	"WlsRxOVP", "WlsRxOCP", "WlsTrxFod", "WlsTrxOCP", "WlsTrxUVLO", "WlsTrxI2cErr"
};
...
struct battery_chg_dev {
	...
	char				manufacturing_date[CHG_DEBUG_DATA_LEN];
	char				first_usage_date[CHG_DEBUG_DATA_LEN];
	bool				battery_auth;
	...
	int quick_rev_chg_stat;
	struct work_struct quick_rev_chg_notify_work;
	bool ut_test_multi_volt;
};
	...
	case BC_XM_STATUS_GET:
		...
		if (len == sizeof(*chg_debug_data)) {
			if (chg_debug_data->type == CHG_ADSP_LOG) {
				...
				ack_set = true;
			} else if (chg_debug_data->type == CHG_CLOUD_DYNAMIC_SHUTDOWN_DATA) {
				...
				ack_set = true;
			} else if (chg_debug_data->type == CHG_MANUFACTURING_DATE_DATA) {
				memset(bcdev->manufacturing_date, '\0', CHG_DEBUG_DATA_LEN);
				memcpy(bcdev->manufacturing_date, chg_debug_data->data, CHG_DEBUG_DATA_LEN);
				ack_set = true;
			} else if (chg_debug_data->type == CHG_FIRST_USAGE_DATE_DATA) {
				memset(bcdev->first_usage_date, '\0', CHG_DEBUG_DATA_LEN);
				memcpy(bcdev->first_usage_date, chg_debug_data->data, CHG_DEBUG_DATA_LEN);
				ack_set = true;
			}
			break;
		}
				...
				size_t len)
{
	...
	case BC_XM_STATUS_UPLOAD:
		...
		break;
	case XM_QUICK_REV_CHG_READY_NOTIFY:
	case XM_QUICK_REV_CHG_OVER_NOTIFY:
		bcdev->quick_rev_chg_stat = notification;
		schedule_work(&bcdev->quick_rev_chg_notify_work);
		break;
	default:
		break;
	}
					...
					struct class_attribute *attr, char *buf)
{
	...
	rc = read_property_id(bcdev, pst, XM_PROP_OPEN_LOADSW);
	if (rc < 0)
		return rc;

	return scnprintf(buf, PAGE_SIZE, "%u\n", pst->prop[XM_PROP_OPEN_LOADSW]);
}
...
static void xm_handle_adsp_dfx_report(struct chg_dfx_report_msg *Context)
{
	...
	case CHG_DFX_SMART_NAVI_TRIG:
		mievent_upload(DFX_ID_CHG_SMART_NAVIGATION, 2, "char", "chgStatInfo", "SmartNaviTrig", "int", "soc", Context->data[0]);
		break;
	case CHG_DFX_SMART_ENDU_SOC_ERR:
		mievent_upload(DFX_ID_CHG_SMART_ENDURANCE_SOC_ERR, 2, "char", "chgErrInfo", "SmartEnduraSocErr", "int", "soc", Context->data[0]);
		break;
	case CHG_DFX_SMART_NAVI_SOC_ERR:
		mievent_upload(DFX_ID_CHG_SMART_NAVIGATION_SOC_ERR, 2, "char", "chgErrInfo", "SmartNaviSocErr", "int", "soc", Context->data[0]);
		break;
	case CHG_DFX_BATT_AUTH_FAIL:
		mievent_upload(DFX_ID_CHG_BATTERY_AUTH_FAIL, 1, "char", "chgErrInfo", "BattAuthFail");
		break;
	case CHG_DFX_SOC_NOT_FULL:
		mievent_upload(DFX_ID_CHG_SOC_NOT_FULL, 4, "char", "chgErrInfo", "SocNotFull", "int", "vbat", Context->data[0],
			"int", "soc", Context->data[1], "int", "rsoc", Context->data[2]);
		break;
	case CHG_DFX_XM_PPS_CHARGE_SLOWLY:
		mievent_upload(DFX_ID_XM_PPS_CHARGE_SLOWLY, 6, "char", "chgStatInfo", "ChargeSlowly", "int", "maxPower", Context->data[0],
			"int", "initialRm", Context->data[1], "int", "endRm", Context->data[2],
			"int", "initialSoc", Context->data[3], "int", "endSoc", Context->data[4]);
		break;
	case CHG_DFX_NON_XM_STANDARD_CHARGER:
		mievent_upload(DFX_ID_NON_XM_STANDARD_CHARGER, 2, "char", "chgStatInfo", "NonStandardCharger", "int", "chgType", Context->data[0]);
		break;
	default:
		;
	}
		...
		struct class_attribute *attr, char *buf)
{
	...
	rc = read_property_id(bcdev, pst, XM_PROP_FG1_CHEMID);
	if (rc < 0)
		return rc;

	return scnprintf(buf, PAGE_SIZE, "%x\n", pst->prop[XM_PROP_FG1_CHEMID]);
}
...
static ssize_t cp_tdie_show(struct class *c,
		struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c,
			struct battery_chg_dev, battery_class);
	...
	rc = read_property_id(bcdev, pst, XM_PROP_CP_TDIE);
	if (rc < 0)
		return rc;

	return scnprintf(buf, PAGE_SIZE, "%u\n", pst->prop[XM_PROP_CP_TDIE]);
}
static CLASS_ATTR_RO(cp_tdie);
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_XM],
						XM_PROP_START_QUICK_REVCHG, val);
	if (rc < 0)
		return rc;
		...
		struct class_attribute *attr, char *buf)
{
	...
	rc = read_property_id(bcdev, pst, XM_PROP_START_QUICK_REVCHG);
	if (rc < 0)
		return rc;

	return scnprintf(buf, PAGE_SIZE, "%d\n", pst->prop[XM_PROP_START_QUICK_REVCHG]);
}
					...
					struct class_attribute *attr, char *buf)
{
	...
	req_msg.property_id = XM_PROP_CHG_DEBUG;
	req_msg.type = CHG_CLOUD_DYNAMIC_SHUTDOWN_DATA;
	req_msg.hdr.owner = MSG_OWNER_BC;
	req_msg.hdr.type = MSG_TYPE_REQ_RESP;
	req_msg.hdr.opcode = pst->opcode_get;
					...
					struct class_attribute *attr, char *buf)
{
	...
	req_msg.property_id = XM_PROP_CHG_DEBUG;
	req_msg.type = CHG_MANUFACTURING_DATE_DATA;
	req_msg.hdr.owner = MSG_OWNER_BC;
	req_msg.hdr.type = MSG_TYPE_REQ_RESP;
	req_msg.hdr.opcode = pst->opcode_get;
	...
	return scnprintf(buf, PAGE_SIZE, "%s", bcdev->manufacturing_date);
}
					...
					struct class_attribute *attr, char *buf)
{
	...
	req_msg.property_id = XM_PROP_CHG_DEBUG;
	req_msg.type = CHG_FIRST_USAGE_DATE_DATA;
	req_msg.hdr.owner = MSG_OWNER_BC;
	req_msg.hdr.type = MSG_TYPE_REQ_RESP;
	req_msg.hdr.opcode = pst->opcode_get;
	...
	return scnprintf(buf, PAGE_SIZE, "%s", bcdev->first_usage_date);
}
...
static int ut_test_parse(struct battery_chg_dev *bcdev)
{
	...
	if (chg_cfg == GL_120W) {
		...
		if (0 == strncmp(prop, "NVT", strlen("NVT"))) {
			...
			if (bcdev->ut_test_multi_volt)
				of_property_read_string(node, "mi,cycle_volt_cn_nvt", &bcdev->cycle_volt);
		}
		if (0 == strncmp(prop, "SUNWODA", strlen("SUNWODA"))) {
			bcdev->temp_term_curr = bcdev->temp_term_curr_cn_sunwoda;
			if (bcdev->ut_test_multi_volt)
				of_property_read_string(node, "mi,cycle_volt_cn_sunwoda", &bcdev->cycle_volt);
		}
	}
		...
		if (0 == strncmp(prop, "NVT", strlen("NVT"))) {
			bcdev->temp_term_curr = bcdev->temp_term_curr_gl_nvt;
			if (bcdev->ut_test_multi_volt)
				of_property_read_string(node, "mi,cycle_volt_gl_nvt", &bcdev->cycle_volt);
		}
		if (0 == strncmp(prop, "SUNWODA", strlen("SUNWODA"))) {
			bcdev->temp_term_curr = bcdev->temp_term_curr_gl_sunwoda;
			if (bcdev->ut_test_multi_volt)
				of_property_read_string(node, "mi,cycle_volt_gl_sunwoda", &bcdev->cycle_volt);
		}
	}
					...
					struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
	static char str[MAX_INFO_LENGTH+1] = {0};
...
static void generate_xm_quick_rev_chg_uevent(struct work_struct *work)
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, quick_rev_chg_notify_work);
	char uevent_buf[MAX_STR_LEN] = { 0 };
	char *envp[2] = { uevent_buf, NULL };
	int rc, notify_val;

	if (bcdev->quick_rev_chg_stat == XM_QUICK_REV_CHG_READY_NOTIFY)
		notify_val = 1;
	else
		notify_val = 0;
	snprintf(uevent_buf, MAX_STR_LEN, "POWER_SUPPLY_REVERSE_QUICK_CHARGE=%d", notify_val);
	rc = kobject_uevent_env(&bcdev->dev->kobj, KOBJ_CHANGE, envp);
	if (rc < 0)
		;
}
...
static int battery_chg_parse_dt(struct battery_chg_dev *bcdev)
{
	...
	bcdev->ut_test_multi_volt = of_property_read_bool(node, "mi,support-ut-test-multi-volt");
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&bcdev->quick_rev_chg_notify_work, generate_xm_quick_rev_chg_uevent);
