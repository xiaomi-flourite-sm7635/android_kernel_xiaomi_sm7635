...
int spidev_open_kern(void)
{
	...
	if (!g_spidev2) {
		pr_debug("spidev2: nothing for dev\n");
		goto err_find_dev;
	}

	if (g_spidev2->users) {
		dev_dbg(&g_spidev2->spi->dev, "open/EBUSY\n");
		status = -EBUSY;
		goto err_find_dev;
	}
...
#if IS_ENABLED(CONFIG_BRCM_XGBE)
	if (g_spidev2 != NULL && g_spidev2 == spidev && spidev->users > 0) {
		pr_debug("spidev2 is in use\n");
		status = -EBUSY;
		goto err_find_dev;
	}
#endif
...
static int spidev_probe(struct spi_device *spi)
{
	...
	if (strcmp(spi->dev.of_node->full_name, "spidev@2") == 0) {
		g_spidev2 = spidev;
		bcm89272_gpio_request(spi);
		check_bcm89272_state_and_reset();
	}
#endif
