...
static void ics_input_rtp_work_routine(struct work_struct *work)
{
    ...
    {
        if (!atomic_read(&haptic_data->is_in_pm_awake))
        {
            ics_dbg("%s: acquire wakeup lock\n", __func__);
            pm_stay_awake(haptic_data->dev);
            atomic_set(&haptic_data->is_in_pm_awake, 1);
        }
        /* boost voltage */
        ...
        if (haptic_data->is_custom_wave == 0)
        {
            ...
            if (ret < 0)
            {
                ...
                if (atomic_read(&haptic_data->is_in_pm_awake))
                {
                    ics_dbg("%s: release wakeup lock\n", __func__);
                    pm_relax(haptic_data->dev);
                    atomic_set(&haptic_data->is_in_pm_awake, 0);
                }
                mutex_unlock(&haptic_data->input_lock);
                return;
            }
                     ...
                     const char *buf, size_t count)
{
    ...
    while (count > 0)
    {
        ...
        if (fifo_in_size == 0)
        {
            ...
            ret = wait_event_interruptible_timeout(haptic_data->kfifo_wait_q, atomic_read(&haptic_data->kfifo_available) == 1, timeout);
            if (ret == -ERESTARTSYS)
            {
                ics_err("%s wake up by signal return erro\n", __func__);
                goto wave_write_exit;
            ...
            {
                ics_err("%s wake up by timeout return erro\n", __func__);
                ret = count;
                goto wave_write_exit;
            }
